...
class ClientService extends BaseApi {
   ...
   Future<http.Response?> editClientInfo(Client client) async{
    try {
    return await api.httpPost('edit_client_info', {'client_id': client.id, 'name': client.name,'email':client.email});
    } catch (e) {
      print(e);
    }
   }
   Future<http.Response?> updatePhone(Client client) async{
    ...
    try {
    return await api.httpPost('edit_client_info', {'client_id': id, 'password': newP,});
    } catch (e) {
      print(e);
    }
   }
